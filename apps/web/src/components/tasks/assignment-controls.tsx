  ...
  const handleTeamChange = async (teamId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          teamId === "unassigned"
            ? `Removed team assignment from ${taskNumber}`
            : `Assigned ${taskNumber} to team`,
        variant: "success",
      });
      onAssignmentChange();
    } catch (error) {
  ...
  const handleAssigneesChange = async (newAssigneeIds: string[]) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Updated assignees for ${taskNumber}`,
        variant: "success",
      });
      onAssignmentChange();
    } catch (error) {
  ...
  const handleClaimTask = async () => {
    ...
    try {
      const res = await fetch(`/api/tasks/${taskId}/assignment`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ claimTask: true }),
      });
      ...
      toast({
        title: "Task Claimed",
        description: `You have joined ${taskNumber}`,
        variant: "success",
      });
      onAssignmentChange();
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {isManagerOrAdmin
            ? "Assign this task to a team or individuals"
      ...
      <CardContent className="space-y-4">
        ...
        {canClaim && (
          ...
          >
            ...
            {currentAssignees?.length > 0 ? "Join This Task" : "Claim This Task"}
          </Button>
        )}
